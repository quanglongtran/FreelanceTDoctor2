...
class ChatRoomControllerV2 extends Controller
{
    ...
    {
        return base64_encode('Tui la Hop');
    }
    ...
    public function __construct(Request $request)
    {
        ...
        if (!empty($request->token)) {
            try {
                $user_id = JWT::decode($request->token, new Key($this->jwt_key, 'HS256'))->user_id;
            } catch (Exception $e) {
                $user_id = false;
            }
            
            if ($user_id) {
                session(['user' => Users::find($user_id)]);
            }
        }
        // echo session('user');
    ...
    function sendPushMessageFirebase($user_id, $title, $url, $body, $icon, $type, $is_show_result = true, $agora_app_id = '', $agora_token = '', $channel_name = '', $agora_client_name = '')
    {
        ...
        if ($user_tokens != null) {
            ...
            $curl = curl_init();
            // return [json_encode(['registration_ids' => $registration_ids])];
    ...
    function sendMessageToNewPatient()
    {
        ...
        if ($patient_user != null) {
            ...
            if ($patient_user->save()) {
                ...
                $type = 'new_message';
                return $this->sendPushMessageFirebase($user_id, $title, $url, $body, $icon, $type, true);
            }
        }
    }
    ...
    function them_benh_nhan_va_bs_khi_chat($rq, $user)
    {
        ...
        if (!is_null($user)) {
            $chatToMember = Users::find((int) $rq->chat_to);
            if (!empty($chatToMember)) {
                switch ($chatToMember->user_type_id) {
                    case 2: // benh nhan chat voi bac si
                        $this->them_benh_nhan_vao_bs_khi_chat($user->user_id, $user->user_id, $chatToMember->user_id); break;
                    case 1: // bac si chat voi benh nhan
                        $this->them_benh_nhan_vao_bs_khi_chat($user->user_id, $chatToMember->user_id, $user->user_id); break;
                }
            }
        }
    }
    public function sendMessage(Request $rq)
    {
        ...
        return $this->them_benh_nhan_va_bs_khi_chat($rq, $user);
        if ($room != null) {
            $room->updated_at = time();
            if ($room->save()) {
                $rq->create_room = $room->id;
