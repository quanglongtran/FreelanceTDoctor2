...
.player-wrapper .player > div {
    height: 500px!important;
}
.player-wrapper .player-name {
    display: none;
}
.is-iphone-webview .icon-call-video, 
.is-iphone-webview .icon-call-audio, 
.is-android-webview .icon-call-video, 
.is-android-webview .icon-call-audio
{
    display: none;
}
</style>
...
// $('.btn-menu-chat').remove();
    ...
    var access_token;
    var call;
    var stringeeUserId = '{{Session::get('user')->user_id}}';
    ...
    function testCall2(toNumber, is_video) {
        ...
        call.makeCall(function (res) {
            if (res.r == 0) {
                ...
                socket.emit('send_to_groups', ['room_'+call.toNumber, {type: 'bat_dau_cuoc_goi_moi', update_name: $('.logo-username').text()}])
            }
        });
    }
    ...
    function func_load_list_message_to_the_message_box(_chat_clone_box, data_send, render_type){
        ...
        socket.on('send_to_groups', function(data){
            ...
            if(allow_video_call){
                if(data.type=='bat_dau_cuoc_goi_moi'){
                    ...
                    if($('#modal-comming-call').hasClass('show')){
                        ...
                        $('.btn-tu-choi-video').attr('data-save', data.fromNumber);
                        $('.list-call-action').attr('data-save', JSON.stringify(data));
                        // $('#incomingcallBox').show();
                        $('#modal-comming-call .modal-title').text('Cuộc gọi đến');
                        // $('#modal-comming-call .right-body h3').html('<span class="incomingCallFrom">'+$('.incomingCallFrom').text()+'</span> đang gọi bạn');
                        $('#modal-comming-call .body-comming-call .avatar').attr('src', '/chatapi/get-avatar/room_'+data.fromNumber);
                        show_hide_call_button_action(true);
                        playSound('/public/v2/sounds/call.mp3', true);                    
                    }        
                }
                if(data.type=='tu_choi_cuoc_goi'){
                    if($('div#modal-comming-call.show').length == 0){
                        return;
                    }
                    alert('Người gọi đang bận');  
                    agora_leave(); 
                    $('#modal-comming-call').modal('hide');  
                    setTimeout(function(){
                ...
                if(data.type=='ket_thuc_cuoc_goi'){
                    if($('div#modal-comming-call.show').length == 0){
                        return;
                    }
                    agora_leave(); 
        ...
        function bi_auto_show_chat_box_after_login(box_id){

            $('.chat-list-right-box').removeClass('active');
            var _closest_box = $('.click-to-start-chat[data-auto-click-to="'+box_id+'"]');
            var chat_to = _closest_box.attr('data-chat-to');
            var create_room = _closest_box.attr('data-chat-room');
            var myName = _closest_box.attr('data-my-name');
            var clientName = _closest_box.attr('data-client-name');
            ...
            if(_closest_box.hasClass('goi-cho-bac-si')){
                setTimeout(function(){
                    $('.chat-box.active textarea').val('Tôi muốn gọi cho '+ _closest_box.attr('data-doctor'));
                    var current_send_button = $('.chat-box.active .btn-send-chat-message');
                    func_send_message_to_room(current_send_button);
                }, 300);
            }
        }
        ...
        $('body').on('click', '.btn-video-call', function(){
            func_click_to_call_agora($(this), true);
        })
        $('body').on('click', '.btn-audio-call', function(){
            func_click_to_call_agora($(this), false);
        })
         ...
         */
        async function subscribe_agora(user, mediaType) {
          const uid = user.uid;
        ...
        function handleUserPublished(user, mediaType) {
          const id = user.uid;
          remoteUsers[id] = user;
          subscribe_agora(user, mediaType);
        }
        ...
        function handleUserUnpublished(user) {
          const agora_id = user.uid;
          delete remoteUsers[agora_id];
          $(`#player-wrapper-${agora_id}`).remove();
        }

        function testHangupx(that) {
            agora_leave();
            var chat_to = $(that).attr('data-save');
            socket.emit('send_to_groups', [chat_to, {type: 'ket_thuc_cuoc_goi', chat_to: chat_to }]);
            $('#modal-comming-call').modal('hide');
            setTimeout(function(){
                window.location.reload();
            }, 200);
        }
        $('.btn-tu-choi-video').click(function(){
            alert('sflsdfsd');
            console.log($(this).attr('data-save'));
        })
        function testRejectx(that){
            console.log(that);
            var chat_to = $(that).attr('data-save');
            socket.emit('send_to_groups', [chat_to, {type: 'tu_choi_cuoc_goi', chat_to: chat_to }]);
            $('#modal-comming-call').modal('hide');
            if(audio_play){
                audio_play.pause();
            }
        ...
        async function testAnswerx(that){
            console.log(that);
            var chat_to = $(that).closest('.list-action-comming-call').find('.btn-tu-choi-video').attr('data-save');
            socket.emit('send_to_groups', [chat_to, {type: 'tra_loi_cuoc_goi', chat_to: chat_to }]);
            if(audio_play){
                audio_play.pause();
            }
            var text = $('.list-call-action').attr('data-save');
        ...
        function func_click_to_call_agora(_that, is_video){
            if($('div#modal-comming-call.show').length > 0){
                return;
            }
            if(!stringeeUserId){
            ...
            $('#call2HangupBtn').attr('data-save', 'room_'+call_to_number);
            ...
            $.ajax({
                ...
                success: async function(res) {
                    ...
                    options.appid = res.agora_app_id;
                    options.token = res.agora_token;
                    options.channel = res.channel_name;
                    // location.reload();
                    // if(res.status == true){
                    //     localStorage.setItem('current_access_token', res.data.access_token);
                    ...
                    // socket.emit('send_to_groups', ['room_'+call_to_number, {type: 'create_new_video_room', chat_to: 'room_'+call_to_number, create_room: create_room, currentID: currentID, clientName: clientName, myName: myName }]);

                    socket.emit('send_to_groups', ['room_'+call_to_number, {type: 'bat_dau_cuoc_goi_moi', update_name: $('.logo-username').text(), create_room: call_to_number, fromNumber: current_id, fromName: myName, options: options }]);
    ...
    <div class="modal-content">
            ...
            <div class="box-show-comming-call-container">
                <div class="body-comming-call">
                    ...
                    <div id="videos">
                        ...
                        <video id="remoteVideox" autoplay playsinline muted style="width: 100%;"></video> -->
      ...
      <div class="modal-footer">
        <div class="list-call-action" style="display: none;">
            <button class="btn btn-primary btn-primary" id="call2HangupBtn" onclick="testHangupx(this)">Kết thúc cuộc gọi</button>            
        ...
        <div id="incomingcallBox" style="display: none; background-color: yellow; padding: 5px; width: 600px;">
            ...
            <button onclick="testAnswerx()">Answer</button>
            <button onclick="testRejectx()">Reject</button>
        </div>
        <div class="list-action-comming-call">
            <button type="button" class="btn btn-primary btn-warning btn-tu-choi-video" data-dismissx="modal" onclick="testRejectx(this)">Từ chối</button>
            <button type="button" class="btn btn-primary btn-primary btn-tra-loi-video" data-dismissx="modal" onclick="testAnswerx(this)">Chấp nhận</button>
        </div>
      </div>
    </div>
  </div>
...
<script type="text/javascript">
    jQuery(document).ready(function($){
        ...
        // $('.footer-doctor').append( userAgent );
        android = /android/.test( userAgent );
        have_version = /version/.test( userAgent );
        ios = /iphone/.test( userAgent );
        have_safari = /safari/.test( userAgent );
        if(ios){
            if(have_safari){
                $('body').addClass('is-iphone-browser');
            }else{
                $('body').addClass('is-iphone-webview');
        ...
        var bi_user_agent = readCookie('bi_user_agent');
        if(!bi_user_agent || bi_user_agent == ''){
            setTimeout(function(){
                $.ajax({
                    type: 'POST',
                    url: '/chatapi/update-useragent',
                    data: {
                        useragent: userAgent
                    },
                    cache: false,
