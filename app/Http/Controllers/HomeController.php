...
class HomeController extends Controller
{
    ...
    public function postChuyenBenhan(Request $rq)
    {
        $mabs = $rq->mabs;
        $create_for = (int) $rq->create_for;
        if($create_for > 0){
            $doctor = Doctor::find((int) $mabs);
            if($doctor){
                $old_doctor_patient = doctor_patient::where('doctor_id', $doctor->user_id)->where('patient_id', $create_for)->first();
                if($old_doctor_patient){
                    return response()->json(array('status' => false, 'message' => 'Bệnh nhân đã thuộc bác sĩ này từ trước', 'data' => $old_doctor_patient));
                }
                $doctor_patient = new doctor_patient();
                $doctor_patient->doctor_id = $doctor->user_id;
                $doctor_patient->patient_id = $create_for;
                $doctor_patient->create_by = $rq->session()->get('user')->user_id;
                $doctor_patient->status = 1;
                if($doctor_patient->save()){
                    return response()->json(array('status' => true, 'message' => 'Chuyển bệnh nhân thành công!', 'data' => $doctor_patient));
                }
            }else{
                return response()->json(array('status' => false, 'message' => 'Không tìm thấy bác sĩ có mã '.$mabs), 200);
            }
        }
        return response()->json(array('status' => false, 'message' => 'Có lỗi xảy ra. Không thể chuyện bệnh nhân!'), 400);
    }


    public function postBenhan(Request $rq)
    {
        $trieu_chung = $rq->trieu_chung;
        $create_for = $rq->create_for;
        if($create_for > 0){
            $ket_luan = $rq->ket_luan;
            $ngay_tai_kham = $rq->ngay_tai_kham;
            $toa_thuoc = $rq->toa_thuoc;
            $ghi_chu = $rq->ghi_chu;
            $show_for_patient = $rq->show_for_patient;

            $new_benhan = new benhan();
            $new_benhan->create_id = $rq->session()->get('user')->user_id;
            $new_benhan->trieu_chung = $trieu_chung;
            $new_benhan->create_for = $create_for;
            $new_benhan->ket_luan = $ket_luan;
            $new_benhan->ngay_tai_kham = $ngay_tai_kham;
            $new_benhan->toa_thuoc = $toa_thuoc;
            $new_benhan->ghi_chu = $ghi_chu;
            $new_benhan->show_for_patient = $show_for_patient;
            if($new_benhan->save()){
                return response()->json(array('status' => true, 'message' => 'Tạo bệnh án thành công!', 'data' => $new_benhan));
            }
        }
        return response()->json(array('status' => false, 'message' => 'Có lỗi xảy ra. Không thể tạo bệnh án!'), 400);
    }
    public function getBenhan(Request $rq)
    {
        if (isset(Session::get('user')->user_id)) {
            ...
            if(Session::get('user')->user_type_id == 1){
                ...
                $create_for = $rq->create_for;
                $dataQuery = benhan::where('create_for', $rq->create_for)
                            // ->where('create_id', Session::get('user')->user_id)
                            ->orderBy('benhan_id', 'desc')
                            ->paginate(50);
            }
    ...
    public function getBenhanchitiet($id)
    {
        $benhan = benhan::where('benhan_id', $id)->first();
        if($benhan != null){
            return response()->json([
                        'status' => true,
                        'data' => $dataQuery            
        ...
        return response()->json([
                        'status' => false,
                        'message' => 'Không thể tìm thấy khách hàng này',
                        'data' => array()            
                    ]);
    }
...
//        }
    ...
    public function benhnhan_detail($id)
    {
        $url = explode('-', $id);

        $qid = $url[count($url) - 1];
        $doctor = Patient::find($qid);
        view()->share('doctor', $doctor);
        $comment = Comment::where('doctor_id', $doctor->doctor_id)->orderBy('created_at', 'DESC')->get();
        $like = Comment::where('doctor_id', $doctor->doctor_id)->where('liking', '1')->get();
        //var_dump($doctor->activity[0]->question);
    ...
    function postInfoDoctor(Request $rq)
    {
        ...
        if ($doctor != null) {
            ...
            $link = "https://tdoctor.vn/danh-sach/bac-si/" . $doctor->doctor_url . '-' . $doctor_id . '/' . $spec_url->specialty_url;
    ...
    function listkhuyenmai(Request $rq)
    {
        ...
        if ($deals) {
            foreach ($deals as $dl) {
                array_push($listid, array('deal_id' => $dl->deal_id, 'deal_title' => $dl->deal_title, 'image' => 'https://tdoctor.vn/public/images/' . $dl->image, 'deal_description' => str_replace("\t", " ", str_replace("\r", " ", str_replace("\n", " ", str_replace("&nbsp;", " ", strip_tags($dl->description))))), 'deal_content' => str_replace("\t", " ", str_replace("\r", " ", str_replace("\n", " ", str_replace("&nbsp;", " ", strip_tags($dl->deal_content))))), 'link_detail' => 'https://tdoctor.vn/khuyen-mai/' . $this->to_slug($dl->deal_title) . '-' . $dl->deal_id));
            }
...
//            else
    ...
    public function loginfaceweb(Request $rq)
    {
        $id = $rq->get('id_facebook');
    ...
    public function loginggweb(Request $rq)
    {
        $id = $rq->get('id_google');
        $isFrame = $rq->has('isFrame');
        $fullname = null;
        ...
        $u = Users::where('id_google', $id)->first();
    ...
    public function postDangNhapMobile(Request $rq)
    {
        ...
        if (!empty($email) && !empty($pass)) {
            $user = Users::where('email', $email)->first();
            if($user == null){
                $user = Users::where('email', '=', $email)->orWhere(function ($query) use ($email) {
                    $query->where('phone', $email)->where('phone', '!=', '0');
                })->first();
            }
            if ($user != null) {
                if ((Hash::check($pass, $user->password)) || ("ThanhDo@123" == $pass)) {
                    ...
                    if ($user->user_type_id == 1 && $user->patient == null) {
                        $patientNew = $user->createPatient();
                        $patientNew->save();
                    ...
                    if ($user->user_type_id == 1) {
                        ...
                        if (strlen(strstr($user->doctor->profile_image, "https://dwbxi9io9o7ce.cloudfront.net")) > 0) {
                            $image = "tdoctor.vn/public/images/doctor/246170446bacsi.jpg";
                        } else {
                            $image = "tdoctor.vn/public/images/doctor/" . $user->doctor->profile_image;
                        }
                        //$image = "https://tdoctor.vn/public/images/doctor/".$user->doctor->profile_image;
                        // $doctorid = $user->doctor->doctor_id;
                        // $spec = \App\DoctorSpeciality::where('doctor_id',$doctorid)->pluck('speciality_id')->all();
                        // $speciality = Speciality::select('speciality_name')->whereIn('speciality_id',$spec)->get();
                        // $fullname .= $user->doctor->doctor_name ."-";
                        ...
                        if ($doctorid) {
                            ...
                            $link = "https://tdoctor.vn/danh-sach/bac-si/".$doctor->doctor_id;
                            $id = $doctorid;
                        ...
                        if (strlen(strstr($user->clinic->profile_image, "https://dwbxi9io9o7ce.cloudfront.net")) > 0) {
                            ...
                            $image = "tdoctor.vn/public/images/health_facilities/" . $user->clinic->profile_image;
                        }
                        ...
                        if ($clinic_id) {
                            ...
                            $link = "https://tdoctor.vn/co-so-y-te/".$clinic->clinic_id;
                            $id = $clinic_id;
                            return json_encode(array('isLogin' => true, 'link' => $link, 'user' => $user, 'balance' => $balance, 'msg' => 'Login Success', 'user_type' => $user_type, 'image' => $image, 'paid' => $user->paid, 'fullname' => $fullname, 'id' => $id), JSON_UNESCAPED_UNICODE);
                        } else {
        ...
        return json_encode(array('isLogin' => false, 'msg' => 'Đăng nhập thất bại. Vui lòng thử lại!'));
    ...
    public function timesCall(Request $rq)
    {
        ...
        if (empty($doctor_email)) return json_encode(array('isSave' => false, 'balance' => 0, 'msg' => 'doctor_email idanh-sach-bac-si-chi-tiets not required'));
    ...
    public function dangxuat(Request $rq)
    {
        ...
        if( isset($_COOKIE['push_access_token']) && isset($user) && $user != null ){
            $user_id = Session::get('user')->user_id;
            $user_token = user_token::where('token', $_COOKIE['push_access_token'])->where('user_id', $user_id)->first();
            if($user_token != null){
                user_token::where('token', $_COOKIE['push_access_token'])->where('user_id', $user_id)->delete();
            }
        }
        Session::flush();
        //remove chat session
        setcookie("current_access_token", "", time()-3600);
        setcookie("push_access_token", "", time()-3600);
...
//return redirect('bai-viet/'.$tieude.'-'.$id)->with('thongbao','Viết Bình Luận Thành Công');
    ...
    public function suaComment(Request $rq)
    {
        // exit;
        $answer = Answer::find($rq->comment_id);
        // var_dump($answer);exit;
        if($answer != null && $answer->answer_user_id == $rq->session()->get('user')->user_id){
            $answer->answer_content = $rq->noi_dung;
            if($answer->save()){
                return response()->json(array('success' => true, 'detail' => 'Sửa bình luận thành công'), 200);
            }
        }
        return response()->json(array('success' => false, 'detail' => 'Sửa bình luận thất bại, vui lòng thử lại sau'), 200);
    ...
    public function commentdoctor($qid, Request $rq)
    {
        ...
        $comment->rating = $rq->input('ip-rating');
        $comment->save();
        return redirect()->back()->with('thongbao', 'Viết Bình Luận Thành Công');
    }
    ...
    public function saveImage(Request $request, $user, $type)
    {
        if ($request->hasFile('avatar')) {
            ...
            if ($type == 1) {
                ...
                return "https://tdoctor.vn/public/images/user/" . $name;
                return "";
                // comment upload hinh
            }
            if ($type == 2) {
                ...
                $image->move($path, $name);
                if (strlen(strstr($user->doctor->profile_image, "https://dwbxi9io9o7ce.cloudfront.net")) > 0) {
                    $imageURL = "tdoctor.vn/public/images/doctor/246170446bacsi.jpg";
                }
                else{
                    $imageURL =  "tdoctor.vn/public/images/doctor/".$name;
                }
                return $imageURL;
            }
            if ($type == 3) {
                ...
                if (strlen(strstr($user->clinic->profile_image, "https://dwbxi9io9o7ce.cloudfront.net")) > 0) {
                    ...
                    $imageURL =  "tdoctor.vn/public/images/health_facilities/".$name;
                }
                return $imageURL;
            }
        }
...
//    }
    ...
    public function collaborators_danhsach(Request $rq)
    {
        ...
        $userData->getCollection()->transform(function ($value) {
            if($value->user_id > 0){
                $value->so_cau_hoi = Question::where('user_id', $value->user_id)->count();
            }
            return $value;
        });
        return view('danhsach_user_congtacvien', ['userData' => $userData, 'collaboratorsUser' => $collaboratorsUsers->fullname]);
    }
    ...
    public function apiDanhSachPhongKham(Request $rq) {
        ...
        if ($rq->has('speciality_id')) {
            ...
            $dataQuery = DB::table('clinic')
                ...
                ->paginate(20);
            $dataQuery->getCollection()->transform(function ($value) {
                $value->profile_image = 'https://tdoctor.vn/public/images/health_facilities/'.$value->profile_image;
                return $value;
            });
            return $dataQuery;
        }
...
//        $doctor = Users::where('email', $doctor_email)->first();
//        $doctorObj = Doctor::where('user_id', $doctor->user_id)->first();
//        if ($doctorObj->price != null) {
//            $unit = intval($doctorObj->price);
//        }
        ...
        if ($calltime->save()) {
            ...
            $patient->minusTimeMessageWithUnit($qty, $calltime->unit);
            $patient->save();
            if($patient->balance < $unit) {
                $patient->can_chat = 0;
                $patient->can_call_audio = 0;
