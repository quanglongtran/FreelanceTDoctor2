		...
		<a class="button secondary weak update-location" href="#">
			<i class="fa fa-crosshairs" aria-hidden="true"></i><i class="fa fa-spinner fa-pulse loader"></i> Định vị
				...
				<!-- <section class="top-list">
					<h3>Tin Tức Tuyển Chọn</h3>
					<ul>
	                    @foreach($baiviets as $baiviet)
	                   
	                	    <?php $baiviet['article_title']; ?>
	                        <li>
	                            <a href="/bai-viet/{{$baiviet->article_url}}-{{$baiviet['article_id']}}" class="image" @if(!empty($baiviet->image)) style="background-image: url(@if(strpos($baiviet->image,'http')===false)/public/images/@endif{{$baiviet->image}});;width:65px;height:40px;display: block;background-size: contain;background-repeat: no-repeat;background-position: center;" @endif title="{{$baiviet['article_title']}}"></a>
