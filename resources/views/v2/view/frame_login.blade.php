<head>
    ...
    <link rel="stylesheet" href="/public/v2/css/style.css?v=1.0.6">
...
<script>
    ...
    function onSignIn(googleUser) {
        ...
        auth2.signOut().then(function () {
            $('.ip-login-gg-id').val(profile.getId());
            $('.ip-login-gg-fullname').val(profile.getName());
            $('.frm-login-gg').submit()
        });
...
<section id="register" style="margin-top: -70px;padding-bottom: 0;">
    <?php $isLoginTag = false;?>
    <div class="span span3 alone pp white-bg rounded">
            ...
            <div class="tab-register @if($errors->has('errorReg') || session('successReg') || !$isLoginTag) active @endif">
                ...
                <form class="frm-submit-register" action="/v2/dang-ky-2" method="post" enctype="miltipart/form-data">
                    ...
                    <div class="group-input">
                        <h3>Họ và tên <span class="field-required">*</span></h3>
                        <div>
                            <input placeholder="Nhập vào Họ và Tên" type="text" name="name" id="name" required>
                        </div>
                    </div>
                    <div class="group-input">
                        <h3>Điện thoại hoặc Email <span class="field-required">*</span></h3>
                        <div>
                    ...
                    <div class="group-input hidden">
                        <h3>Email <span class="field-required hidden">*</span></h3>
                        <div>
                    ...
                    <div class="group-input text">
                        <h3>Mật khẩu <span class="field-required">*</span></h3>
                        <div>
                            <input type="password" name="password"
                                   placeholder="Mật khẩu để đăng nhập lần sau">
                        </div>
                    </div>
                    <div class="group-input text" style="border: 1px solid #818181; padding: 10px">
                        <h3>Mã code (Không bắt buộc)</h3>
                        <div>
                            <input type="text" name="refer_id" value="<?php if(isset($_REQUEST['rc'])){ echo $_REQUEST['rc']; } ?>">
                        </div>
                        <?php
                        $is_doctor_refer = 'checked="checked"';
                        $is_clinic_refer = '';
                        if(isset($_REQUEST['rt']) && isset($_REQUEST['rc']) && $_REQUEST['rc'] != ''){ 
                            if($_REQUEST['rt'] == 3){
                                $is_doctor_refer = '';
                    ...
                    <br>
                    <div class="fcenter" style="clear: both; margin-top:0">
...
{{--                        </button>--}}
            ...
            <div class="tab-login @if($errors->has('errorlogin') || $isLoginTag) active @endif">
                <form method="post" action="/v2/dang-nhap-2" name="login">
                    ...
                    <div class="fcenter" style="margin-top: 12px">
                        <img class="btn-login-with-facebook" src="/public/v2/img/login-with-facebook.PNG" alt=""
                             style="cursor: pointer; width: 208px;">
                    </div>
                    <div class="fcenter" style="margin-top: 12px">
                        <div style="cursor: pointer; width: 208px" class="g-signin2" data-onsuccess="onSignIn"></div>
                    </div>
                </form>
                ...
                <form class="frm-login-gg" method="post" action="/logingg" name="loginface">
                    <input type="hidden" name="_token" value="{{csrf_token()}}"/>
                    <input type="hidden" name="isFrame" value="true"/>
                    <input type="hidden" name="next" value="{{Request::get('next')}}"/>
                    <input class="ip-login-gg-id" name="id_google" required="" type="hidden" value="">
                    <input class="ip-login-gg-fullname" name="fullname" required="" type="hidden" value="">
                    <div hidden class="group-input text">
                        <h3>GOTO</h3>
                        <div>
                            <input class="goto" type="text" name="goto" placeholder="">
...
<script>
    $(document).ready(function () {
        ...
        if(window.location.href.includes('type=login')){
            $('.login-tab').click();
        }
        setHeightEqualWidth();
